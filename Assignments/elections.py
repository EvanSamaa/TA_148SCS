...
class Election:
    ...
    def update_results(self, riding: str, party: str, votes: int) -> None:
        ...
        """
        # TODO: implement this method!
        ## see if the party is already in the party list
        has_riding = False
        for i in range(0, len(self._ridings)):
            if self._ridings[i] == riding:
                has_riding = True
                break
        if not has_riding:
        ...
        ## see if the party is already in the party list
        has_party = False
        for i in range(0, len(self._parties)):
            if self._parties[i] == party:
                has_party = True
                break
        if not has_party:
            self._parties.append(party)

        # add vote to the riding and party accordingly
        if has_riding:
            try:
            ...
            self._results[riding]: Dict[str, int] = {}
            self._results[riding][party] = votes
    ...
    def results_for(self, riding: str, party: str) -> Optional[int]:
        ...
        # TODO: implement this method!
        try:
            val = self._results[riding][party]
        except:
            val = None
        # val = self._results[riding][party]
        return val
    def riding_winners(self, riding: str) -> List[str]:
        ...
        for i in range(0, len(parties)):
            if self._results[riding][parties[i]] >= max_votes:
                max_votes = self._results[riding][parties[i]]
        for i in range(0, len(parties)):
            if self._results[riding][parties[i]] == max_votes:
                rtl.append(parties[i])
        return rtl
    def popular_vote(self) -> Dict[str, int]:
        ...
        for r in self._ridings:
            parties_for_that_riding = list(self._results[r].keys())
            for p in parties_for_that_riding:
                vote[p] = vote[p] + self._results[r][p]
        return vote
    def party_seats(self) -> Dict[str, int]:
        ...
        # print(self._parties)
        for r in self._ridings:
            winner = self.riding_winners(r)
            if len(winner) == 1:
                seats[winner[0]] = seats[winner[0]] + 1
        return seats
...
class Jurisdiction:
    ...
    def __init__(self, name: str) -> None:
        ...
        # TODO: implement this method!
        self._name = name
        self._elections: Dict[date, Election] = {}
    ...
    def read_results(self, year: int, month: int, day: int,
        ...
        """
        # TODO: implement this method!
        date_of_e = date(year, month, day)
        new_election = Election(date_of_e)
        new_election.read_results(input_stream)
        self._elections[date_of_e] = new_election
    ...
    def party_wins(self, party: str) -> List[date]:
        ...
        # TODO: implement this method!
        rtv = []
        for key in list(self._elections.keys()):
            election = self._elections[key]
            winner = election.election_winners()
            for p in winner:
                if p == party:
                    rtv.append(key)
        return rtv
    ...
    def party_history(self, party: str) -> Dict[date, float]:
        ...
        True
        """
        wins = self.party_wins(party)
        rtl = {}
        for windate in wins:
            e = self._elections[windate]
            seats = e.party_seats()
            total_seats = 0
            for p in list(seats.keys()):
                total_seats = total_seats + seats[p]
            percentage : float = seats[party]/total_seats
            rtl[windate] = percentage
        return rtl
        # TODO: implement this method!
    ...
    def riding_changes(self) -> List[Tuple[Set[str], Set[str]]]:
        ...
        if len(dates) == 0:
            ...
            for i in range(0, len(dates)-1):
                riding0 = set(self._elections[dates[i]].ridings_recorded())
                riding1 = set(self._elections[dates[i+1]].ridings_recorded())
                dif = riding0.union(riding1).difference(riding1.intersection(riding0))
                added = set([])
                removed = set([])
