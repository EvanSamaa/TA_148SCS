...
class GameBoard:
    ...
    width: int
    height: int
    _board: List[List[List[Character]]] # 0: empty,
    ...
    def place_character(self, c: Character) -> None:
        ...
        """
        # TODO Task #1
        grid = self.at(c.x, c.y)
        if len(grid) == 0:
            self._board[c.y][c.x] = [c]
        elif c.__class__ == Raccoon and len(self._board[c.y][c.x]) == 1 and self._board[c.y][c.x][0].__class__ == GarbageCan:
        ...
        for y in range(0, self.height):
            ...
            for x in range(0, self.width):
                ...
                if len(slot) == 0:
                    ...
                    if slot[0].__class__ == Raccoon:
                        curr_row.append("R")
                    elif slot[0].__class__ == Player:
                        curr_row.append("P")
                    elif slot[0].__class__ == SmartRaccoon:
                        curr_row.append("S")
                    elif slot[0].__class__ == GarbageCan:
                        g: GarbageCan = slot[0]
                        if g.locked:
                            curr_row.append("C")
                        else:
                            curr_row.append("O")
                    elif slot[0].__class__ == RecyclingBin:
                        curr_row.append("B")
                else:
                    curr_row.append("@")
    ...
    def __str__(self) -> str:
        ...
        """
        # TODO Task #1
        gb_list = self.to_grid()
        str_rep = ""
        for row in gb_list:
            for item in row:
                str_rep = str_rep + item
            str_rep = str_rep +"\n"
        str_rep = str_rep[:-1]
        return str_rep
    ...
    def get_adjacent_list(self, x, y):
        rtl = []
        if x-1 >= 0:
            char_list = self.at(x-1, y)
            for item in char_list:
                if item.__class__ == RecyclingBin:
                    rtl.append([x-1, y])
        if y-1 >= 0:
            char_list = self.at(x, y-1)
            for item in char_list:
                if item.__class__ == RecyclingBin:
                    rtl.append([x, y-1])
        if x+1 < self.width:
        ...
        if y+1 < self.height:
            ...
            for item in char_list:
                if item.__class__ == RecyclingBin:
                    rtl.append([x, y+1])
        return rtl
    def recurse_check(self, queue:queue.Queue, bool_board):
        if queue.empty():
            return
        curr = queue.get()
        bool_board[curr[1]][curr[0]] = True
        neighbours = self.get_adjacent_list(curr[0], curr[1])
        for n in neighbours:
            if bool_board[n[1]][n[0]] == False:
                queue.put(n)
        self.recurse_check(queue, bool_board)
    ...
    def adjacent_bin_score(self) -> int:
        ...
        for y in range(0, self.height):
            ...
            for x in range(0, self.width):
                row.append(False)
            flag_board_base.append(row)
        max_count = -1
        for x in range(0, self.width):
            for y in range(0, self.height):
                char_list = self.at(x, y)
                for item in char_list:
                    if item.__class__ == RecyclingBin:
                        q = queue.Queue()
                        q.put([x, y])
                        bool_map = copy.deepcopy(flag_board_base)
                        self.recurse_check(q, bool_map)
                        count = 0
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        if new_y < 0 or new_y >= self.board.height:
            return False
        char_list = self.board.at(new_x, new_y)
        if len(char_list) > 0:
            # if there is any thing there then return false
            for item in char_list:
                if item.__class__ != RecyclingBin:
                    return False
            # alternatively it must be a recycle bin:
            for item in char_list:
                if item.__class__ == RecyclingBin:
                    item.move(direction)
        self.x = new_x
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        if self.x-1 < 0:
            ...
            char_list = self.board.at(self.x-1, self.y)
            for char in char_list:
                if (char.__class__ == Player or char.__class__ == RecyclingBin or char.__class__ == Raccoon
                or char.__class__ == SmartRaccoon):
                    left = False
        if self.y-1 < 0:
            up = False
        else:
            char_list = self.board.at(self.x, self.y-1)
            for char in char_list:
                if (char.__class__ == Player or char.__class__ == RecyclingBin or char.__class__ == Raccoon
                or char.__class__ == SmartRaccoon):
                    up = False
